...
const EmotionAnalyzer = () => {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl glass-panel h-80 lg:h-[calc(100vh-350px)] flex flex-col neo-glow">
      {error && (
      ...
      <div className="absolute inset-0 pointer-events-none border border-purple-500/20 rounded-2xl"></div>
      
      <div className="absolute top-2 right-2 z-10">
        <Badge variant="outline" className="bg-black/30 backdrop-blur-md text-white border-purple-500/50 px-3 py-1">
          <ScanFace size={14} className="mr-1 text-purple-400" /> Emotion Scanner
        </Badge>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-black/60 backdrop-blur-sm p-3 flex justify-between items-center border-t border-white/10">
        ...
        <Badge variant="outline" className="bg-purple-500/20 text-white border-purple-400/50 px-3 py-1.5">
      ...
      <div className="absolute inset-0 border-[8px] border-transparent rounded-2xl bg-clip-padding p-1">
        <div className="w-full h-full rounded-lg border border-purple-500/30 pointer-events-none"></div>
      </div>
    </div>
  );
};
